...
function init(){
  ...
  directional.add(params.directional, 'helper').onChange(value =>{  if (lights.directionalHelper) lights.directionalHelper.visible = value});
  ...
  spot.add(params.spot, 'helper').onChange(value =>{ if (lights.spotHelper) lights.spotHelper.visible = value });
  ...
  area.add(params.area, 'width').min(1).max(15).onChange( value => { 
    ...
    if (lights.areaHelper) lights.areaHelper.update();
  });
  area.add(params.area, 'height').min(1).max(15).onChange( value => { 
    lights.area.height = value;
    if (lights.areaHelper) lights.areaHelper.update();
  });
  area.add(params.area, 'helper').onChange(value => { if (lights.areaHelper) lights.areaHelper.visible = value });
