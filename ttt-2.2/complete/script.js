class SphereHelper extends THREE.Mesh {
  constructor(obj, radius=10, center=new THREE.Vector3(), col=0xffffff){
    super();
  ...
  update(){
    ...
    this.position.copy(this.obj.position).add(this.center);
  }
}
...
function init(){
  ...
  const helper1 = new THREE.BoxHelper(player);
  helpers.push(helper1);
  scene.add(helper1);
  const helper2 = new SphereHelper(player, 2, new THREE.Vector3(0, 0.8, 0));
  scene.add(helper2);
...
function playerControl(forward, turn){
    const pos = player.position.clone();
        ...
        console.log(intersect[0].distance.toFixed(1) + " " + player.userData.blocked);
			}
		}
